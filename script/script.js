...
const popupCloseImg = document.querySelectorAll('.popup__close_img');
...
profileEditButton.addEventListener('click', function() {openPopup(editProfile)});
popupCloseEdit.addEventListener('click', function() {closePopup(editProfile)});
...
function manageCard(name, link) {
  ...
  const clear = element.querySelectorAll('.element__delete');
  ...
  // Закрытие картинки
  popupCloseImg.forEach(function (item) {
    item.addEventListener('click', function () {
...
profileAddButton.addEventListener('click', function() {openPopup(addElement)});
popupCloseAdd.addEventListener('click', function() {closePopup(addElement)});
